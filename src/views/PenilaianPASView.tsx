...
export const PenilaianPASView: React.FC = () => {
  ...
  const [pasScores, setPasScores] = useState<Record<string, { teori: number; praktik: number; remedial?: number }>>({});
  ...
  useEffect(() => {
    ...
    classStudents.forEach((s) => {
      ...
      map[s.id] = {
        teori: existing?.pasScore ? Math.min(50, Math.floor(existing.pasScore * 0.5)) : 42,
        praktik: existing?.pasScore ? Math.min(50, Math.ceil(existing.pasScore * 0.5)) : 41,
        remedial: existing?.pasScore && existing.pasScore < kktp ? kktp : undefined,
      };
    });
    setPasScores(map);
  }, [selectedClassId, subject, students.length]);

  const handleScoreChange = (studentId: string, field: 'teori' | 'praktik' | 'remedial', val: number) => {
  ...
  const getFinalPAS = (scores: { teori: number; praktik: number; remedial?: number }) => {
    const raw = (scores.teori || 0) + (scores.praktik || 0);
    if (scores.remedial && scores.remedial > raw) {
      return Math.min(kktp, scores.remedial);
    }
    return raw;
  };
  ...
  const handleSavePAS = () => {
    ...
    classStudents.forEach((s) => {
      const sc = pasScores[s.id] || { teori: 40, praktik: 40 };
      const finalPas = getFinalPAS(sc);
      ...
      if (idx >= 0) {
        ...
        updatedGrades.push({
          ...
          ptsScore: 80,
          pasScore: finalPas,
          finalScore: Math.round(((80 * 2 + 80 + finalPas) / 4) * 10) / 10,
          predicate: finalPas >= 90 ? 'A' : finalPas >= 80 ? 'B' : finalPas >= 70 ? 'C' : 'D',
        });
      }
    });
  ...
  const handleExportExcel = () => {
    const exportData = classStudents.map((s, idx) => {
      const sc = pasScores[s.id] || { teori: 0, praktik: 0 };
      const finalPas = getFinalPAS(sc);
      return {
        No: idx + 1,
        NIS: s.nis,
        'Nama Siswa': s.name,
        'Nilai Teori / SAS (Max 50)': sc.teori,
        'Nilai Praktik / Portofolio (Max 50)': sc.praktik,
        'Remedial (Jika Ada)': sc.remedial || '-',
        'Nilai Akhir PAS': finalPas,
        'Status KKTP': finalPas >= kktp ? 'Tuntas' : 'Remedial',
      };
    });
    ExportService.exportToExcel(exportData, `Nilai_PAS_${selectedClass?.name}_${subject}`);
  };
  ...
  const handleExportWord = () => {
    ...
    classStudents.forEach((s, idx) => {
      ...
      rows += `
        <tr>
          ...
          <td style="text-align:center;">${sc.teori}</td>
          <td style="text-align:center;">${sc.praktik}</td>
          <td style="text-align:center;">${sc.remedial || '-'}</td>
          <td style="text-align:center; font-weight:bold; background-color:#f1f5f9;">${finalPas}</td>
          <td style="text-align:center; font-weight:bold; color:${finalPas >= kktp ? '#16a34a' : '#dc2626'};">${finalPas >= kktp ? 'Tuntas' : 'Remedial'}</td>
        </tr>
      `;
    });
    ...
    const bodyHtml = `
      <div style="margin-bottom:15px;">
        <strong>PENILAIAN AKHIR SEMESTER (PAS / SUMATIF AKHIR SEMESTER - SAS)</strong><br>
      ...
      <table>
        <thead>
          <tr>
            ...
            <th>Ujian Teori (50)</th>
            <th>Ujian Praktik/Portofolio (50)</th>
            <th>Remedial</th>
            <th>Nilai Akhir PAS</th>
            <th>Status</th>
          </tr>
        </thead>
        <tbody>${rows}</tbody>
    ...
    ExportService.exportToWord(`DAFTAR NILAI PAS - ${selectedClass?.name}`, bodyHtml, schoolProfile, `Nilai_PAS_${selectedClass?.name}`);
  };
  ...
  const handlePrintPdf = () => {
    ...
    classStudents.forEach((s, idx) => {
      ...
      rows += `
        <tr>
          ...
          <td style="text-align:center;">${sc.teori}</td>
          <td style="text-align:center;">${sc.praktik}</td>
          <td style="text-align:center; font-weight:bold;">${finalPas}</td>
          <td style="text-align:center;">${finalPas >= kktp ? 'Tuntas' : 'Remedial'}</td>
        </tr>
      `;
    });
    ...
    const bodyHtml = `
      <div style="margin-bottom:10px;">
        <strong>Nilai Penilaian Akhir Semester (PAS / SAS):</strong> ${subject} | Kelas: ${selectedClass?.name} (KKTP: ${kktp})
      </div>
      <table>
        <thead>
          <tr>
    ...
    ExportService.printPdfPreview(`NILAI PAS ${selectedClass?.name}`, bodyHtml, schoolProfile);
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-slate-900 p-6 rounded-2xl border border-slate-800 shadow-sm">
        <div className="flex items-center space-x-2.5">
          ...
          <div>
            <h1 className="text-xl font-black text-white tracking-tight">9. Penilaian Akhir Semester (PAS / SAS)</h1>
            <p className="text-xs text-slate-400">
              Pengolahan skor asesmen sumatif akhir semester (teori dan praktik/portofolio) untuk penentuan nilai rapor.
            </p>
          </div>
        </div>
          ...
          >
            {saveSuccess ? (
              <>
                ...
                <span>Nilai PAS Tersimpan!</span>
              </>
            ) : (
              <>
                <Save className="w-4 h-4" />
                <span>Simpan Semua Nilai PAS</span>
              </>
            )}
          </button>
        </div>
          ...
          <table className="w-full text-left text-xs">
            <thead className="bg-slate-950/80 text-slate-400 font-semibold border-b border-slate-800">
              <tr>
                ...
                <th className="py-3 px-3 w-24 text-center">Teori / SAS (50)</th>
                <th className="py-3 px-3 w-24 text-center">Praktik (50)</th>
                <th className="py-3 px-3 w-24 text-center">Remedial</th>
                <th className="py-3 px-4 w-24 text-center bg-purple-950/40 text-purple-300">Nilai PAS</th>
                ...
                <tr>
                  <td colSpan={8} className="py-12 text-center text-slate-400">
                    <div className="max-w-md mx-auto space-y-3">
                      <div className="p-3 bg-purple-950/40 border border-purple-800/40 rounded-2xl inline-block text-purple-400">
                  ...
                  <tr key={s.id} className="hover:bg-slate-800/40 transition">
                    ...
                    <td className="py-2.5 px-3 text-center">
                      <input
                        ...
                        max="50"
                        value={sc.teori}
                        onChange={(e) => handleScoreChange(s.id, 'teori', parseInt(e.target.value))}
                    ...
                    <td className="py-2.5 px-3 text-center">
                      <input
                        ...
                        max="50"
                        value={sc.praktik}
                        onChange={(e) => handleScoreChange(s.id, 'praktik', parseInt(e.target.value))}
                        className="w-16 text-center px-2 py-1 bg-slate-950 border border-slate-700 rounded-lg text-white font-mono focus:outline-none focus:border-indigo-500"
                      />
                    </td>
                    <td className="py-2.5 px-3 text-center">
                    ...
                    <td className="py-2.5 px-4 text-center font-bold font-mono bg-purple-950/20 text-purple-300 text-sm">
                      {finalPas}
                    </td>
